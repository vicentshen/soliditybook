...
可以将数据保存在能映射到区块级别的特殊的索引数据结构中。这个功能叫做日志，在Solidity中，为了实现事件的功能。合约无法访问日志数据，但是这可以在区块链之外高效访问。因为一些日志数据是在`bloom filters`中，所以可以通过高效且加密安全的方式去搜索这些数据，所以那些没有下载区块链全部数据的网络节点（`轻客户端`）也可以找到这些日志。

###创建

合约甚至可以通过一些特殊的操作码来创建其他合约。那些创建调用和普通的消息调的唯一区别是payload数据会被执行，结果数据被保存成了代码，调用者或创建者在栈上接受到了新的合约的地址。

###自毁

将代码从区块链上移除的唯一方法是调用`selfdestruct`操作。地址上保存的剩余的与台币将会被发送到一个特定的目标地址，然后存储和代码将会被移除。